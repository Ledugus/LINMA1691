...
def do_test(test):
    input_file = "devoir_1/tests/input" + str(test) + ".txt"
    expected_output_file = "devoir_1/tests/output" + str(test) + ".txt"

    read_and_solve_tests(input_file, "devoir_1/tmp/output" + str(test) + ".txt")
    ok, errorLine = compare_files(
        expected_output_file, "devoir_1/tmp/output" + str(test) + ".txt"
    )
    if not ok:
        print(
            "Difference between output and expected output for test : "
            + str(test)
            + " on line : "
            + str(errorLine)
        )
